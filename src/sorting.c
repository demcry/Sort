...
int main(int argc, char* argv[])
{
	...
	if (par == 1)
		...
		if (par == 2)
			printf("sort ignorecase\n"); //sorting ignorecase
		else
			if (par == 3)
				printf("sort ignorecase + reverse\n"); //sorting ign + rev
			else
				if (par == 4)
					printf("sort alphabet + ignorecase\n");// sorting alpha + ign
				else
					if (par == 5)
						printf("sort random\n"); // sorting random
					else
	...
	else
	{
		if (argc == 4)
		{
			...
			if (file1 != NULL)
			{
				...
				{
					fclose(file2);
					*flagfile2 = 2; //est' file2
				}
				else return -4; //problem with file
			}
			else return -4; //problem with file
		}
		else
		{
			FILE *file1 = fopen(argv[2], "r+");
			if (file1 == NULL) return -4; //problem with file
			else *flagfile2 = 1; //net file2
		}
		if ((argv[1][0] == '-'))
		{
